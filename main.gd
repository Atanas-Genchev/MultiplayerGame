...
var player_dictonary : Dictionary[int,Player] = {}
var player_name_dictonary: Dictionary[int,String] = {}
...
func _ready():
	...
	multiplayer_spawner.spawn_function = func(data):
		...
		player_dictonary[data.peer_id] = player
		return player
...
func peer_ready(display_name : String):
	...
	player_name_dictonary[sender_id] = display_name
	multiplayer_spawner.spawn({ 
		"peer_id": sender_id,
		"display_name": player_name_dictonary[sender_id],
		"is_respawning": false
	})
...
func respawn_dead_peers():
	...
	for peer_id in dead_peers:
		...
		multiplayer_spawner.spawn({ 
			"peer_id": peer_id,
			"display_name": player_name_dictonary[peer_id],
			"is_respawning": true
			})
	dead_peers.clear()
...
func _on_peer_disconected(peer_id : int):
	if player_dictonary.has(peer_id):
		var player := player_dictonary[peer_id]
		if is_instance_valid(player):
			player.kill()
		player_dictonary[peer_id].kill
